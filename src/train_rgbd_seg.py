...
def build_cache(root_dir, split):
    ...
    # Auto-detect label mode by inspecting the first PNG. Legacy labels are
    # uint8 in {0,1,2,3,4,5} where 0=bg and 1..5=harness classes (Phase 4 v2
    # render output). Phase 11 ``label3/`` PNGs are uint8 in {0,1,2} where
    # 0=backdrop, 1=wire, 2=objects — already trainer-ready, no gt_transform
    # shift needed. The detector trips on max>=3 (legacy) vs max<=2 (3-way).
    sample_base = file_names[0].split("/")[1].replace(".png", "")
    sample_lbl = cv2.imread(
        os.path.join(label_dir, sample_base + ".png"), cv2.IMREAD_GRAYSCALE,
    )
    three_way_labels = bool(sample_lbl is not None and int(sample_lbl.max()) <= 2)
    print(f"    label mode: {'three_way (label3, no gt_transform)' if three_way_labels else 'legacy (gt_transform: 0->255, 1..5->0..4)'}")
    ...
    for i, fn in enumerate(file_names):
        ...
        depth_arr[i] = d
        if three_way_labels:
            # Already {0=backdrop, 1=wire, 2=objects}. Trainer reads as-is.
            label_arr[i] = lbl
        else:
