...
Phase 9 / KD-student deliverable.
...
def parse_args():
    ...
    p.add_argument("--resume", default=None,
                   help="Path to an epoch_<N>.pth / best_model.pth produced by "
                        "this trainer. Warm-restart: continues IN the checkpoint's "
                        "own directory (no new timestamped dir), restores model "
                        "weights, resumes at epoch N+1, restores optimizer + "
                        "scheduler state if present (else re-inits and "
                        "fast-forwards the LR schedule to the matching global "
                        "step, resetting optimizer momentum), and restores the "
                        "best-IoU watermark. Used to recover from a server crash "
                        "mid-run.")
    return p.parse_args()
...
def main():
    ...
    # On --resume: continue IN the resumed checkpoint's own directory (do NOT
    # mint a new timestamped dir) so logs/ckpts/report append in place.
    if args.resume is not None:
        if not os.path.isfile(args.resume):
            raise FileNotFoundError(f"--resume not found: {args.resume}")
        results_dir = os.path.dirname(os.path.abspath(args.resume))
    else:
    ...
    # ── Warm-restart from a crash (--resume) ──
    # Checkpoints written by THIS run dir before the crash carry model weights +
    # epoch (+ metrics for best_model.pth). New checkpoints (post-this-code) also
    # carry optimizer/scheduler state. We restore model weights, jump the epoch
    # counter to N+1, restore (or re-init + fast-forward) the LR schedule to the
    # matching global step, and recover the best-IoU watermark. When optimizer
    # state is absent (the live job's ckpts), AdamW moments reset — negligible at
    # the small post-warmup LRs where crashes are recovered.
    if args.resume is not None:
        rs = torch.load(args.resume, map_location="cpu", weights_only=False)
        rs_state = rs["model_state_dict"] if (isinstance(rs, dict) and "model_state_dict" in rs) else rs
        rs_state = {k.replace("module.", ""): v for k, v in rs_state.items()}
        tgt = student.module if isinstance(student, DDP) else student
        ...
        if isinstance(rs, dict) and rs.get("optimizer_state_dict") is not None:
            try:
                optimizer.load_state_dict(rs["optimizer_state_dict"])
                opt_restored = True
            except Exception as e:
                if rank == 0:
                    print(f"    WARNING: optimizer_state_dict present but failed to load ({e}); re-initialised")
        if isinstance(rs, dict) and rs.get("scheduler_state_dict") is not None:
            try:
                scheduler.load_state_dict(rs["scheduler_state_dict"])
                sched_restored = True
            except Exception as e:
                if rank == 0:
                    print(f"    WARNING: scheduler_state_dict present but failed to load ({e}); fast-forwarding")
        if isinstance(rs, dict) and rs.get("scaler_state_dict") is not None:
            try:
                scaler.load_state_dict(rs["scaler_state_dict"])
            except Exception:
                pass
        if not sched_restored:
            # Fast-forward LambdaLR (per-batch stepped) to the matching step so
            # the resume LR equals what it would have been at start_epoch.
            for _ in range(global_step):
                scheduler.step()

        # Restore best-metric watermark: prefer ckpt 'metrics', else report.json, else 0.
        rs_metrics = rs.get("metrics", {}) if isinstance(rs, dict) else {}
        if rs_metrics:
            best_iou_dlo = float(rs_metrics.get("iou_dlo", 0.0) or 0.0)
            ...
            if os.path.isfile(rj):
                try:
                    with open(rj) as f:
                        prev = json.load(f)
                    best_iou_dlo = float(prev.get("best_iou_dlo", 0.0) or 0.0)
                    best_miou = float(prev.get("best_miou", 0.0) or 0.0)
                except Exception:
                    pass
        ...
        if rank == 0:
            ...
            print(f"    optimizer restored={opt_restored}, scheduler restored={sched_restored}"
                  + ("" if opt_restored else "  (AdamW momentum RESET — expected for legacy/live ckpts)"))
            print(f"    restored best watermark: IoU(DLO)={best_iou_dlo:.4f} mIoU={best_miou:.4f}")
            print(f"    missing keys ({len(miss)}); unexpected ({len(unexp)})")

    if rank == 0:
        torch.cuda.reset_peak_memory_stats()

    epoch = start_epoch - 1  # so the final report is well-defined if already complete
