...
def main() -> int:
    ...
    parser.add_argument("--views", type=str, nargs="+", default=None,
                        help="View slot names to tile across. Default: "
                             "auto-detect from smoke_dir/metadata.json "
                             "(phase9 → view0..view5; v2 → front/back/...).")
    ...
    if args.views is not None:
        ...
        import json
        meta_path = args.smoke_dir / "metadata.json"
        views = ["front", "back", "left", "right", "top", "bottom"]
        if meta_path.is_file():
            try:
                meta = json.loads(meta_path.read_text())
                vm = meta.get("views", {})
                if isinstance(vm, dict) and vm.get("view_slot_names"):
                    views = list(vm["view_slot_names"])
                elif meta.get("dataset_mode") == "phase9":
                    views = [f"view{i}" for i in range(6)]
            except Exception:
                pass
