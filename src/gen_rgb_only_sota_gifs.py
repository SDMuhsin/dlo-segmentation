...
def load_model(ckpt_path, device, backbone=BACKBONE_DEFAULT):
    ...
    # Honour the ckpt's saved num_classes (3-way Phase 11 ckpts have
    # num_classes=3; legacy binary ckpts have num_classes=2 or no config).
    num_classes = int(cfg.get("num_classes") or saved_args.get("num_classes")
                      or NUM_CLASSES)
