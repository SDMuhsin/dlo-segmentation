#!/usr/bin/env python
"""Build the P43 bright-disambiguation negative co-train dataset.

IDENTICAL to the P39 cutout-neg co-train build (``src/build_p39_cutoutneg_cotrain.py``)
EXCEPT the negative composites are pulled from the ``_p43neg`` dir (bright/pale
disambiguation cutouts) instead of ``_p39neg``. Same current-best base
(synth phase15_wirefree + MovingCables + realtextured), same 1800-frame 1:1
substitution, same set-id offsets (synth 0, MovingCables +1000, real-textured
+3000..+6000 x4 oversample, stride 1000). FULL DOSE (all 1800).

The ONLY difference vs the base build: for the chosen synth train bases we
symlink the composited RGB/Label (and Depth) from the _p43neg dir instead of the
clean phase15_wirefree frame — keeping the SAME remapped set-id basename so the
line in train.txt is identical to the base (and to P39, since placement /
...
NEG = "data/dformer_dataset_phase15_wirefree_p43neg"
MC_OFFSET = 1000
RT_OFFSET = 3000
RT_REPLICA_STRIDE = 1000
NEG_SUFFIX = "_p43neg"
SUB_LIST_NAME = "p43_substituted_basenames.txt"
...
def link_subset(src, dst, bases, offset, sub_map=None, neg_dir=NEG):
    ...
    pull RGB/Label/Depth from the substitute dir (neg_dir) at base+NEG_SUFFIX instead, but
    ...
    for base in bases:
        ...
        for sub in ("RGB", "Depth", "Label"):
            if use_sub:
                sp = os.path.join(neg_dir, sub, base + NEG_SUFFIX + ".png")
...
def main():
    ...
    ap.add_argument("--out-dir", default="data/dformer_dataset_p43_brightdisambig_cotrain")
    ...
    sub_list_path = os.path.join(args.neg, SUB_LIST_NAME)
    ...
    print("symlinking synth (with p43neg substitution on chosen train frames) ...")
    ...
    print(f"  substituted {len(subbed)} synth train frames with {NEG_SUFFIX} composites")
    ...
    with open(os.path.join(out, "test.txt"), "w") as f:
        f.write("\n".join(val) + "\n")
    with open(os.path.join(out, SUB_LIST_NAME), "w") as f:
        f.write("\n".join(sorted(subbed)) + "\n")

    print("\n=== P43 BRIGHT-DISAMBIG CO-TRAIN BUILT ===")
    print(f"out-dir: {out}")
    print(f"train: {len(train)}  = synth {len(s_tr)} + MC {len(m_tr)} + RT {len(r_tr)}")
    print(f"val:   {len(val)}")
    print(f"substituted ({NEG_SUFFIX}) train frames: {len(subbed)}")
    print(f"substituted-list -> {os.path.join(out, SUB_LIST_NAME)}")
