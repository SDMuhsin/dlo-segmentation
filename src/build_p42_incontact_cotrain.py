#!/usr/bin/env python
"""Build the P42 in-contact fixture-negative co-train dataset.
...
frames are replaced 1:1 by their ``_p42neg`` composited versions (real-photo object
cutouts pasted AT WIRE ENDS / in-contact, labeled background). Drift-controlled
substitution: train.txt count is UNCHANGED (== base 16623), total wire density preserved.

Mirrors ``src/build_p39_cutoutneg_cotrain.py`` exactly (same set-id offsets:
...
symlink the composited RGB/Label (and Depth) from the _p42neg dir instead of the clean
...
FULL DOSE: substitute all 1800 (matching P39 full-dose).
...
NEG = "data/dformer_dataset_phase15_wirefree_p42neg"
MC_OFFSET = 1000
RT_OFFSET = 3000
...
def link_subset(src, dst, bases, offset, sub_map=None, neg_dir=NEG):
    ...
    pull RGB/Label/Depth from the substitute dir (neg_dir) at base+'_p42neg' instead, but
    ...
    for base in bases:
        ...
        for sub in ("RGB", "Depth", "Label"):
            if use_sub:
                sp = os.path.join(neg_dir, sub, base + "_p42neg.png")
...
def main():
    ...
    ap.add_argument("--out-dir", default="data/dformer_dataset_p42_incontact_cotrain")
    ...
    sub_list_path = os.path.join(args.neg, "p42_substituted_basenames.txt")
    ...
    print("symlinking synth (with p42neg substitution on chosen train frames) ...")
    ...
    print(f"  substituted {len(subbed)} synth train frames with _p42neg composites")
    ...
    print("\n=== P42 IN-CONTACT CO-TRAIN BUILT ===")
    print(f"out-dir: {out}")
    print(f"train: {len(train)}  = synth {len(s_tr)} + MC {len(m_tr)} + RT {len(r_tr)}")
    print(f"val:   {len(val)}")
    print(f"substituted (p42neg) train frames: {len(subbed)}")
